...
A single `publishCatalog` server action that turns funnel state into a live catalog in one call: it creates (or reuses) the restaurant configuration with slug and QR code, imports all extracted categories and dishes, and returns the public slug, QR code, and menu URL.
